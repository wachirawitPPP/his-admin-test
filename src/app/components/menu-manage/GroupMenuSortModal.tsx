...
const SortableRow = ({ item }: { item: MenuType }) => {
  ...
  return (
    <TableRow ref={setNodeRef} style={style}  className={`dark:text-white hover:bg-lightprimary hover:text-primary  dark:hover:bg-lightprimary`} {...attributes} {...listeners}>
...
const GroupMenuSortModal: React.FC<GroupMenuSortModalProps> = ({
  ...
  useEffect(() => {
    ...
    const sortedMenu = [...menuList].sort((a, b) => a.mng_sort - b.mng_sort);
    setItems(sortedMenu);
  }, [menuList]);
